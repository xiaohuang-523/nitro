...
if __name__ == '__main__':
    ...
    MISSING_TOOTH_NUMBER = [18, 19, 20, 21, 22, 23, 31]
    ...
    #for i in arch_ios.existing_tooth_list:  # used for algorithm verification (count missing tooth as well)
    ...
    # transform splint ground geometry to ios space
    trans_init = np.eye(4)
    ...
    splint_fiducial_ground_rigid = transpose_pc(splint_fiducial_ground, np.linalg.inv(trans_rigid0))
    splint_fiducial_ground_transformed = transpose_pc(splint_fiducial_ground_rigid, modify_matrix)
    splint_fiducial_ios_transfomred = transpose_pc(splint_fiducial_ios, modify_matrix)
    ...
    delta = splint_fiducial_ios_transfomred[0:11,:] - splint_fiducial_ground_transformed[0:11,:]
    ...
    for i in fiducial_list_yomiplan:
        yomi_plan_fiducial_ios.append(splint_fiducial_ground_transformed[i,:])
    ...
    for i in range(np.shape(splint_fiducial_ground)[0]):
        splint_ground.add_pyramid(i, splint_fiducial_ground_transformed[i, :])
        splint_ios.add_pyramid(i, splint_fiducial_ios_transfomred[i, :])
    ...
    for i in NEIGHBOUR_TOOTH:
        ctl_target.append(arch_ct.get_tooth(i).points)
        ctl_source.append(arch_ct_in_ios.get_tooth(i).points)
    ...
    mtx = np.matmul(mtx, np.linalg.inv(modify_matrix2))
    mtx = np.matmul(mtx, trans_rigid)

    #check_parameters = Yomikin.Yomi_parameters(np.matmul( np.linalg.inv(golden_transformation), trans_rigid))
    #trans_check = affine_registration.rigid_registration(check_parameters, yomi_plan_fiduial_fiducialFrame, yomi_plan_fiducial_ct)
    #trans_check_mtx = Yomikin.Yomi_Base_Matrix(trans_check)
    ...
    # test_fiducial_frame_fiducials = np.array([[-50, -40, -20],
    #                                           [-50, -40, 20],
    #                                           [-50, 0, -20],
    #                                           [-50, 0, 20],
    #                                           [1, -40, -20],
    ...
    test_fiducial_frame_fiducials = np.array([
        [0, 0, 0],
        [10, 0, 0],
        [0, 20, 0],
        [0, 0, 30],
        [-92.0799, -26.3569, - 11.8082],
        [-64.5862, -85.9349, - 11.5782]
    ])

    landmark_ct = np.array([118.2, 78.2, 4.8])
    implant_ct = np.array([[105.4768, 54.2752, 9.8],
                           [105.4312, 54.3512, 7.8]])



    implant_x = (implant_ct[1,:] - implant_ct[0,:])/np.linalg.norm(implant_ct[1,:] - implant_ct[0,:])
    implant_o = implant_ct[0,:]
    ...
    implant_ct = np.vstack((implant_ct, implant_o + implant_ct_delta))
    implant_ct = np.vstack((implant_ct, implant_o - 20 * implant_ct_delta))
    implant_ct = np.vstack((implant_ct, implant_o - 40 * implant_ct_delta))
    implant_ct = np.vstack((implant_ct, implant_o + 20 * implant_x))
    implant_ct = np.vstack((implant_ct, implant_o - 20 * implant_x))
    ...
    test_ct_frame_fiducials = transpose_pc(test_fiducial_frame_fiducials, np.linalg.inv(mtx))
    ...
    yomi_plan_fiducial_ct_test = np.copy(yomi_plan_fiducial_ct)
    yomi_plan_fiducial_ct_test[:,1] = yomi_plan_fiducial_ct[:,0]
    yomi_plan_fiducial_ct_test[:,0] = yomi_plan_fiducial_ct[:,1]
    yomi_plan_fiducial_ct_test[:, 2] = 304*0.2 - yomi_plan_fiducial_ct[:,2]
    ...
    draw_registration_result_points_array(ctl_target, ctl_source, trans_rigid)
