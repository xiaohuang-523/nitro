...
if __name__ == '__main__':
    ...
    arch_ct_in_ios_no_correction = fe.FullArch(MISSING_TOOTH_NUMBER, 'CT', 'IOS')
    ...
    # for debugging, do registration without splint correction
    local_ICP.do_local_registration(TRANS_INIT, THRESHOLD_ICP, RMS_LOCAL_REGISTRATION, arch_ios, arch_ct, DEBUG=0)
    for i in arch_ct_in_ios_no_correction.existing_tooth_list:
        print('transform tooth ', i)
        points_ICP_tem_no_correction = transpose_pc(arch_ct.get_tooth(i).points, arch_ct.get_tooth(i).local_ICP_transformation)
        tooth_feature_ICP_tem_no_correction = fe.ToothFeature(points_ICP_tem_no_correction, i, 'CT', 'IOS')
        arch_ct_in_ios_no_correction.add_tooth(i, tooth_feature_ICP_tem_no_correction)
    trans_init2 = arch_ios.get_tooth(17).local_ICP_transformation

    ctl_ios2 = []
    for i in NEIGHBOUR_TOOTH:
        ctl_ios2.append(arch_ct_in_ios_no_correction.get_tooth(i).points)
    ctl_ios2 = combine_pc(ctl_ios2)
    ...
    if path.exists(RESULT_TEM_BASE + 'optimized_rigid_transformation_2nd.csv'):
        trans_rigid2 = Yomiread.read_csv(RESULT_TEM_BASE + 'optimized_rigid_transformation_2nd.csv', 4, 4)
    else:
        affine_rigid_part2 = affine_registration.rigid_registration(rigid_init_parameter2, ctl_ct, ctl_ios2)
        trans_rigid2 = Yomikin.Yomi_Base_Matrix(affine_rigid_part2)
        Yomiwrite.write_csv_matrix(RESULT_TEM_BASE + 'optimized_rigid_transformation_2nd.csv', trans_rigid2,
                                   fmt='%0.8f')
...
# ------ for drill fixture ------ #
    landmark_ct = yomi_plan_fiducial_ct[12,:]
    implant_ct = np.array([[(800 - 285.9) * 0.2, 264.834 * 0.2, (50 + 304-200)*0.2],
                           [(800 - 286.881) * 0.2, 265.624 * 0.2, (50 + 304-210)*0.2]])
    landmark_ct_2 = np.array([(800-230)*0.2, 390*0.2, (50+304-283)*0.2])
    landmark_ct_3 = np.array([(800-334.926)*0.2, 496.589*0.2, (50+304-279)*0.2])
    landmark_ct_4 = np.array([(800-468.541)*0.2, 502.463*0.2, (50+304-273)*0.2])
    landmark_ct_tooth = np.array([(800-257)*0.2, 228*0.2, (50+304-169)*0.2])


    implant_x = (implant_ct[1,:] - implant_ct[0,:])/np.linalg.norm(implant_ct[1,:] - implant_ct[0,:])
    implant_o = implant_ct[1,:]
    ...
    implant_transformation_ct[0:3,3] = implant_o
    print('implant_transformation_ct is', implant_transformation_ct)
    implant_transformation_fiducial = np.matmul(mtx, implant_transformation_ct)
    print('implant_transformation_fiducial is', implant_transformation_fiducial)
    print('mtx is', mtx)
    print('golden is', golden_transformation)
    print('fiducial frame is', fiducial_frame)

    implant_ct_delta = 0.5 * implant_z
    for i in range(40):
        implant_ct = np.vstack((implant_ct, implant_o + i * implant_ct_delta))
        implant_ct = np.vstack((implant_ct, implant_o - i * implant_ct_delta))
        #implant_ct = np.vstack((implant_ct, implant_o - 40 * implant_ct_delta))
        implant_ct = np.vstack((implant_ct, implant_o + i * implant_x))
        implant_ct = np.vstack((implant_ct, implant_o - i * implant_x))
    #implant_ct = np.vstack((implant_ct, implant_o - 40 * implant_x))
    #implant_ct = np.vstack((implant_ct, implant_o + 40 * implant_x))
    implant_fiducial = transpose_pc(implant_ct, mtx)

    landmark_fiducial_new = np.vstack((landmark_ct_tooth, landmark_ct_tooth + 20 * implant_y))
    landmark_fiducial_new = np.vstack((landmark_fiducial_new, landmark_ct_tooth + 40 * implant_y))
    landmark_fiducial_new = np.vstack((landmark_fiducial_new, landmark_ct_tooth - 40 * implant_y))
    landmark_fiducial_new = np.vstack((landmark_fiducial_new, landmark_ct_tooth - 20 * implant_y))
    landmark_fiducial_new_fiducial = transpose_pc(landmark_fiducial_new, mtx)
    ...
    landmark_fiducial_tooth = transpose_pc(landmark_ct_tooth, mtx)

    print('check landmark and implant in ct space')
    check_pc4 = np.vstack((arch_ct.allpoints, landmark_fiducial_new))
    check_pc4 = np.vstack((check_pc4, implant_ct))
    ...
    check_pc5 = np.vstack((allpoints_fiducial_space, landmark_fiducial_new_fiducial))
    check_pc5 = np.vstack((check_pc5, implant_fiducial))
    ...
    new_mesh = mesh.Mesh.from_file("G:\\My Drive\\Project\\IntraOral Scanner Registration\\IOS_scan_raw_data\\IOS Splint\\demo_scan\\2nd_splint\\scan3\\5242021-Demo_2nd_splint_trial3-lowerjaw.stl")
    ...
    #check_centroids[:, 1] = check_centroids_ori[:, 0]
    #check_centroids[:, 0] = check_centroids_ori[:, 1]
    #check_centroids[:, 2] = 304*0.2 - check_centroids_ori[:, 2]
    ...
    Yomiwrite.write_csv_matrix(RESULT_TEM_BASE + 'implant_in_fiducial_space.csv', implant_fiducial,
                               fmt='%0.8f')
    Yomiwrite.write_csv_matrix(RESULT_TEM_BASE + 'implant_transforamtion_in_fiducial_space.csv', implant_transformation_fiducial,
                               fmt='%0.8f')
    Yomiwrite.write_csv_matrix(RESULT_TEM_BASE + 'transforamtion_from_fiducial_space_to_ct.csv', np.linalg.inv(mtx),
                               fmt='%0.8f')
