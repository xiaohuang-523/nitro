...
def plot_boxplot(data1, data2, data3):
    ...
    for i in range(NUMBER_TARGETS):
        target.append([data1[:,i], data2[:,i], data3[:,i]])
    ...
    #hP, = plot([1, 1], 'm-')
    #legend(group_labels)
    legend((hB, hR, hG),('3 fiducials', '4 fiducials', '5 fiducials'))
    hB.set_visible(False)
    hR.set_visible(False)
    hG.set_visible(False)
    #hP.set_visible(False)

    ax.set_title('TRE with ' + np.str(BIAS_MEAN) + ' mm random tolerance')
    #savefig('boxcompare.png')
    show()
...
for i in range(3):
    ...
    for region in range(3):
        value_95_file = np.str(3+i) + "_region_" + np.str(BIAS_MEAN) + "_tolerance_value_95_region_" + np.str(region + 1) + ".txt"
    ...
    all_value95_all.append(all_value95_random_noise)




plot_boxplot(all_value95_all[0], all_value95_all[1], all_value95_all[2])