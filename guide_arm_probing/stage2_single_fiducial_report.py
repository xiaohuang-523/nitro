...
if __name__ == "__main__":
    path = "G:\\My Drive\\Project\\HW-9232 Registration method for edentulous\\Edentulous registration error analysis\\Stage2\\Results\\Original Target\\"
    ...
    if CHECK_TARGET != 0:
        ...
        for region in range(N_REGION):
            ...
            value_95 = Yomiread.read_txt(path + value_95_file)
            points = Yomiread.read_csv(path + points_file, 3, -1)
            print(value_95[0])
            print('shape of value_95 is', np.shape(value_95))
            all_value95 = np.concatenate((all_value95, value_95[0]))
            #all_value95.append(value_95[0])
            all_points = np.vstack((all_points, points))

        #all_value95 = all_value95[1:,:]
        all_points = all_points[1:,:]
        all_value95_check = np.asarray(all_value95[2:])
