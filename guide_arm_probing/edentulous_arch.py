...
def divide_region_surface_matching(angle_range, point):
    # check with region does the point belong to
    theta_idx = ap.check_interval_idx_single_value(point[1]*180/np.pi, angle_range)
...
class Edentulous_arch:
    ...
    def divide_arch_individual_target_modified_angle_seg(self, D_RADIUS, D_ANGLE, D_HEIGHT, individual_defined_range, angle_span):
        ...
        for region in self.all_regions_cartesion:
            print('region shape is', np.shape(region))
            print('self.default_fiducial is', self.default_fiducial)
            center = np.sum(np.asarray(region), axis=0) / len(region)
            self.default_fiducial.append(center)
        self.default_fiducial = np.asarray(self.default_fiducial)

    # divide arch into different regions based on settings for surface matching
    # Arguments:
    #   1.  angle span of each segment (fiducial region)
    #
    def divide_arch_individual_target_surface_matching(self, angle_span):
        print('Perform surface matching registration')
        check_angle_range = []
        check_angle_range.append(self.target_origins_cylindrical[0,1] * 180 / np.pi - 20)
        for i in range(len(self.target_origins_cylindrical[:, 1]) - 1):
            check_angle_range.append((self.target_origins_cylindrical[i, 1]
        ...
        for i in range(self.n_target + 1):
            ...
            print('check_angle_range_tem is', check_angle_range_tem)
            for j in range(len(self.all_points_cylindrical)):
                x = divide_region_surface_matching(check_angle_range_tem, self.all_points_cylindrical[j, :])
                if x == 1:
                    surface_points.append(self.all_points_cartesian[j, :])

        for i in range(len(surface_points)):
            a = surface_points[i] == [0., 0., 0.]
            if a.all() == True:
                print('found all zeros')
                surface_points.pop(i)
                break
        self.surface_points = np.asarray(surface_points)

    # remove regions from the arch
    def remove_regions(self,remvoe_region_idx):
        # rearrange list in descending order
        # https://www.programiz.com/python-programming/methods/list/sort
        new_region_idx = ap.sort_list(remvoe_region_idx, False)
        print('new_region_idx is', new_region_idx)
        for idx in new_region_idx:
            idx_tem = np.int(idx)
            self.default_fiducial = np.delete(self.default_fiducial, idx_tem, axis=0)
            self.all_regions_cartesion.pop(idx_tem)
            self.all_regions_cylindrical.pop(idx_tem)
