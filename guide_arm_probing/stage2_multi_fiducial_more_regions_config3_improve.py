# Further tests using configuration III for multi-target simulation
# Check script stage2_multi_fiducial_more_regions.py
# by Xiao Huang @ 08/02/2021
...
def check_tre(targets, fiducials):
    ...
    if np.ndim(targets) == 1:
        ...
        for j in range(len(targets)):
            value = sa.fit_models_np_plot_mean_std(tre_total[:, j])
            value_95.append(value[0])
            mean.append(value[1])
            stdev.append(value[2])
...
if __name__ == "__main__":
    ...
    for i in range(len(arch1.all_regions_cartesion)):
        ...
        for point in arch1.all_regions_cartesion[i]:
            ...
            value_95_tem, mean_tem, stdev_tem = check_tre(targets_original, fiducials_new)
            value_95.append(value_95_tem)
            mean.append(mean_tem)
            stdev.append(stdev_tem)
        ...
        min_mean.append(np.min(mean, axis=0))
        min_stdev.append(np.min(stdev, axis=0))
        print('maximum 95% value is', max_value_95)
        print('minimum 95% value is', min_value_95)
        print('minimum mean value is', min_mean)
        print('minimum stdev value is', min_stdev)
