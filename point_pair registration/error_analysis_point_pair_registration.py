...
if __name__ == '__main__':
    ...
    target_measurement_file = "Raw_data\\target_fiducial_measurements.txt"
    points = Yomiread.read_csv(base + target_measurement_file, 4, 10, flag=0)
    ...
    # print('fiducials zero is', fiducials_zero)
    # print('fiducials original is', fiducials_original)
    # all_vertex = region_points.read_5_regions()
    ...
    if N_REGION == 5:
        ...
        fiducials_original[2, :] = fiducials_zero[3, :]
        fiducials_original[3, :] = fiducials_zero[2, :]
        print('fiducials are ', fiducials_original)
        all_vertex = region_points.read_5_regions()
    elif N_REGION == 4:
        ...
        print('fiducials are ', fiducials_original)
        all_vertex = region_points.read_4_regions()
    elif N_REGION == 3:
        ...
        fiducials_original = points[:, 1:4]
        print('fiducials are ', fiducials_original)
        all_vertex = region_points.read_3_regions()
    ...
    for i in range(N_REGION):
        ...
        Yomiwrite.write_csv_matrix(base + "Results\\" + np.str(N_REGION) + "_region_" + np.str(RAN_MEAN) + "_random_tolerance_and " + np.str(BIAS_MEAN) + "_bias_tolerance_points_region_" + np.str(i+1) + ".txt", all_vertex[i])
